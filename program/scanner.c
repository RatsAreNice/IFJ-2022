...
//     to. (aby sa dalo potom testovat na EOF)] eof,
  ...
  while (1) {
    switch (state) {
      case start:
        ...
        if (skip ==
            1) {  // mod na zaciatku -> nic nepreskakuje, ocakava prolog1.
          if (a == '<') {
            a = getchar();
            if (a == '?') {
              a = getchar();
        ...
        } else if (skip == 0) {  // mod po prologu -> preskakuje whitespaces a
                                 // komentare.
          while (isspace(a) ||
        ...
        if ((a >= 97 && a <= 122) || a == '_' ||
            (a >= 65 && a <= 90)) {  // male pismeno / velke pismeno / _
          state = IDfunkcie;
        } else if (a == '$') {
          state = premenna1;
        } else if (a == '"') {
          ...
          return make_token(
              lbracket, "(");  // preslo by sa do stavu "(" kde by sa iba
                               // vytvoril token lbracket. pre zjednodusenie sa
                               // do stavu neprechadza a token sa vytvori priamo
        } else if (a == ':') {
          return make_token(
              colon, ":");  // preslo by sa do stavu "(" kde by sa iba vytvoril
                            // token lbracket. pre zjednodusenie sa do stavu
                            // neprechadza a token sa vytvori priamo
        } else if (a == ')') {
      ...
      case IDfunkcie:
        str = "";
        i = 0;
        while ((a >= 97 && a <= 122) || a == '_' || (a >= 65 && a <= 90) ||
               (a >= 48 && a <= 57)) {
        ...
        if (strcmp(str, "string") == 0 || strcmp(str, "float") == 0 ||
            strcmp(str, "int") == 0) {
        ...
        return make_token(ID_function,
                          str);  // ak to nieje klucove slovo, je to ID funkcie
        break;
      ...
      case string1:
        ...
        while (
            a != '"' && a > 31 &&
            a != '$') {  // osetrit skoncenie dolarom / <31 a to ze druhy znak
                         // nieje ani jeden esc. sequence / esc seq na oktalove
          int cflag = 0;
          z = 0;
          ...
          if (a == 92) {  // osetrenie escape sekvencii
            ...
            if (a == 'n') {
              ...
              for (int j = 1; j >= 0; j--) {
                a = getchar();
                if ((a <= 57 && a >= 48) || (a >= 65 && a <= 70) ||
                    (a >= 97 && a <= 102)) {  // hexadecimalna cislovka
                  z = z + hextoint(a) * pwr(16, j);
                  b = a;
                } else {
                  if (j == 1) {  // prvy znak neni hexadecimalna cislovka : do
                                 // stringu treba pridat \x
                    size_t len = strlen(str);
                    ...
                    i++;
                    break;
                  } else {  // druhy znak neni hexadecimalna cislovka : do
                            // stringu treba pridat \xb
                    size_t len = strlen(str);
              ...
              if (z != -1) {  // ak sa jednalo o escape sekvenciu \xdd pridava
                              // sa iba jeden znak, a to vypocitane a
                a = z;
              } else {
                cflag = 1;
              }
              ...
              for (int j = 2; j >= 0; j--) {
                ...
                if (a >= 48 && a <= 55) {  // hexadecimalna cislovka
                  ...
                  if (j == 2) {  // prvy znak neni oktalova cislovka : do
                                 // stringu treba pridat '\'
                    size_t len = strlen(str);
                    char *str2 = malloc(len + 1 + 1);
                    strcpy(str2, str);
                    if (i > 0) {
                    ...
                    i++;
                    break;
                  } else if (j == 1) {  // druhy znak neni oktalova cislovka :
                                        // do stringu treba pridat \b
                    size_t len = strlen(str);
                    char *str2 = malloc(len + 1 + 2);
                    strcpy(str2, str);
                    if (i > 0) {
                    ...
                    i++;
                    break;
                  } else {  // posledny znak neni oktalova cislovka : do stringu
                            // treba pridat \cb
                    size_t len = strlen(str);
                    char *str2 = malloc(len + 1 + 3);
                    strcpy(str2, str);
              ...
              if (z != -1) {  // ak sa jednalo o escape sekvenciu \ddd pridava
                              // sa iba jeden znak, a to vypocitane a
                a = z;
              } else {
                cflag = 1;  // inak sa posledny nacitany znak znova spracuje
                            // (mohlo by ist o " alebo nepovoleny znak)
              }
            } else {  //'\'a
              size_t len = strlen(str);
              char *str2 = malloc(len + 1 + 1);
      ...
      case type1:
        ...
        if (a == '>') {  // nasiel sa epilog ?>
          ...
          if (a != '\n') {
            ungetc(a, stdin);
          }  // ak nasledujuci znak je druhy znak windows newlinu/jediny znak
             // linux newlinu, skonzumuje sa
      ...
      case lt1:
        ...
        a = getchar();
        if (a != ' ' && a != '\n' && a != '\t' &&
            a != '\r') {  // toto by sa nikdy nemalo stat
          fprintf(stderr, "chybna znacka <?php");
          exit(1);
        }
        return make_token(
            prolog1, "<?php ");  // ak sa nahodou vyskytne druhy prolog v kode
                                 // ten uz vyriesi syntakticka analyza :D
        break;
      case declare:
        str = "";
        i = 0;
